...
-- sf :: _
sf (Filter{limitF}) = case limitF of 
       Just (Limit (fromIntegral -> x)) -> runSelectReturningList 
                                         . select 
                                         . limit_ x 
                                         . nub_ 
                                         . orderBy_ (desc_ . _time) 

       Nothing -> runSelectReturningList 
                . select 
                . limit_ 10000000 
                . nub_ 
                . orderBy_ (desc_ . _time) 
...
fetch db (Filter (Just (Ids tx@(all isHex32 -> True))) _ _ _ _ _ _ _ _ ) = do 
    hx <- mapM lookupEid $ mapMaybe toHex32 tx
    pure . mapMaybe (qw . _con) . catMaybes $ hx
    where 
    lookupEid :: Hex32 -> IO (Maybe (Ev Identity)) 
    lookupEid = runBeamSqlite db 
              . runSelectReturningOne 
              . lookup_ (_events spec') 
              . EvId 
              . wq
        
fetch db ff = mapMaybe (qw . _con) <$> runBeamSqlite db (sf ff $ getQf ff) 
